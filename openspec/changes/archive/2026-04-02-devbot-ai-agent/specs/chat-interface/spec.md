...
The system SHALL provide a web route that displays the chat interface for interacting with DevBot.

#### Scenario: Access chat page

- **WHEN** a user navigates to `/chat`
- **THEN** the system SHALL display the chat interface
- **AND** it SHALL load the most recent conversation or create a new one
...
The system SHALL provide an HTTP endpoint that accepts user messages, processes them through DevBot, and returns the AI response.

#### Scenario: Submit message successfully

- **WHEN** a POST request is made to `/chat/message` with message content
