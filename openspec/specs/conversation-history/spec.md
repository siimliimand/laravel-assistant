...
The system SHALL provide an HTTP endpoint that returns a list of all conversations sorted by most recent first.

#### Scenario: Retrieve conversation list

- **WHEN** a GET request is made to `/chat/conversations`
- **THEN** the system SHALL return a JSON array of conversations
